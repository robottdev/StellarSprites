/**
 * LibNoise.Unity port (Perlin, Voronoi, RidgedMultifractal, planar Noise2D).
 * Source: https://github.com/ricardojmendez/LibNoise.Unity (LGPL)
 */
export const QualityMode = { Low: 0, Medium: 1, High: 2 };
...
const RANDOMS = new Float64Array([
...
function noiseCorner(x) {
  return x > 0.0 ? Math.trunc(x) : Math.trunc(x) - 1;
}

function interpolateLinear(a, b, position) {
  return (1.0 - position) * a + position * b;
}

function mapCubicSCurve(value) {
  return value * value * (3.0 - 2.0 * value);
}

function mapQuinticSCurve(value) {
  const a3 = value * value * value;
  const a4 = a3 * value;
  const a5 = a4 * value;
  return 6.0 * a5 - 15.0 * a4 + 10.0 * a3;
}

function makeInt32Range(value) {
  if (value >= 1073741824.0) {
    return (2.0 * ieeeRemainder(value, 1073741824.0)) - 1073741824.0;
  }
  if (value <= -1073741824.0) {
    return (2.0 * ieeeRemainder(value, 1073741824.0)) + 1073741824.0;
  }
...
function gradientNoise3D(fx, fy, fz, ix, iy, iz, seed) {
  let i = (GEN_X * ix + GEN_Y * iy + GEN_Z * iz + GEN_SEED * seed) >>> 0;
  i ^= i >>> GEN_SHIFT;
  i &= 0xff;
  const idx = i << 2;
  const xvg = RANDOMS[idx];
  const yvg = RANDOMS[idx + 1];
  const zvg = RANDOMS[idx + 2];
  return (xvg * (fx - ix) + yvg * (fy - iy) + zvg * (fz - iz)) * 2.12;
}

export function gradientCoherentNoise3D(x, y, z, seed, quality) {
  const x0 = noiseCorner(x);
  const x1 = x0 + 1;
  const y0 = noiseCorner(y);
  const y1 = y0 + 1;
  const z0 = noiseCorner(z);
  const z1 = z0 + 1;
  let xs, ys, zs;
  ...
  } else if (quality === QualityMode.Medium) {
  ...
  const s = seed | 0;
  let n0 = gradientNoise3D(x, y, z, x0, y0, z0, s);
  let n1 = gradientNoise3D(x, y, z, x1, y0, z0, s);
  const ix0 = interpolateLinear(n0, n1, xs);
  n0 = gradientNoise3D(x, y, z, x0, y1, z0, s);
  n1 = gradientNoise3D(x, y, z, x1, y1, z0, s);
  const ix1 = interpolateLinear(n0, n1, xs);
  const iy0 = interpolateLinear(ix0, ix1, ys);
  n0 = gradientNoise3D(x, y, z, x0, y0, z1, s);
  n1 = gradientNoise3D(x, y, z, x1, y0, z1, s);
  const ix0b = interpolateLinear(n0, n1, xs);
  n0 = gradientNoise3D(x, y, z, x0, y1, z1, s);
  n1 = gradientNoise3D(x, y, z, x1, y1, z1, s);
  const ix1b = interpolateLinear(n0, n1, xs);
  const iy1 = interpolateLinear(ix0b, ix1b, ys);
  return interpolateLinear(iy0, iy1, zs);
}

export function valueNoise3DInt(x, y, z, seed) {
  let n = (GEN_X * x + GEN_Y * y + GEN_Z * z + GEN_SEED * seed) & 0x7fffffff;
  n = (n >> 13) ^ n;
  return (Math.imul(n, Math.imul(Math.imul(n, n), 60493) + 19990303) + 1376312589) & 0x7fffffff;
}
...
export class Perlin {
  constructor(frequency, lacunarity, persistence, octaves, seed, quality) {
  ...
  getValue(x, y, z) {
    ...
    for (let i = 0; i < this.octaveCount; i++) {
      const nx = makeInt32Range(x);
      const ny = makeInt32Range(y);
      const nz = makeInt32Range(z);
      const seed = (this.seed + i) & 0xffffffff;
...
export class RidgedMultifractal {
  constructor(frequency, lacunarity, octaves, seed, quality) {
    this.frequency = frequency;
    this.lacunarity = lacunarity;
    this.octaveCount = octaves;
    this.seed = seed;
    ...
    this.gain = 2.0;
    this.offset = 1.0;
    this.weights = new Float64Array(30);
    this.updateWeights();
  }
  ...
  updateWeights() {
    let f = 1.0;
    for (let i = 0; i < 30; i++) {
      this.weights[i] = Math.pow(f, -this.exponent);
      f *= this.lacunarity;
    }
  }
  ...
  getValue(x, y, z) {
    ...
    for (let i = 0; i < this.octaveCount; i++) {
      ...
      const ny = makeInt32Range(y);
      const nz = makeInt32Range(z);
      const seed = (this.seed + i) & 0x7fffffff;
...
export class Voronoi {
  constructor(frequency, displacement, seed, distance) {
    this.frequency = frequency;
  ...
  getValue(x, y, z) {
    ...
    y *= this.frequency;
    z *= this.frequency;
    const xi = noiseCorner(x);
    const iy = noiseCorner(y);
    const iz = noiseCorner(z);
    let md = 2147483647.0;
    let xc = 0, yc = 0, zc = 0;
    for (let zcu = iz - 2; zcu <= iz + 2; zcu++) {
      for (let ycu = iy - 2; ycu <= iy + 2; ycu++) {
        for (let xcu = xi - 2; xcu <= xi + 2; xcu++) {
          const xp = xcu + valueNoise3D(xcu, ycu, zcu, this.seed);
          const yp = ycu + valueNoise3D(xcu, ycu, zcu, this.seed + 1);
          const zp = zcu + valueNoise3D(xcu, ycu, zcu, this.seed + 2);
          const xd = xp - x;
          const yd = yp - y;
          const zd = zp - z;
    ...
    if (this.useDistance) {
      ...
      const yd = yc - y;
      const zd = zc - z;
      v = Math.sqrt(xd * xd + yd * yd + zd * zd) * SQRT3 - 1.0;
    } else {
      v = 0.0;
    }
    return v + this.displacement * valueNoise3D(Math.floor(xc), Math.floor(yc), Math.floor(zc), 0);
  }
}

export function generatePlanarNormalized(module, width, height) {
  const xe = width;
  const ze = height;
  const xd = xe / (width - 1);
  const zd = ze / (height - 1);
  const ucWidth = width + 2;
  const ucHeight = height + 2;
  const uc = new Float32Array(ucWidth * ucHeight);
  let xc = 0;
  for (let x = 0; x < ucWidth; x++) {
    let zc = 0;
    for (let y = 0; y < ucHeight; y++) {
      uc[x + y * ucWidth] = module.getValue(xc, 0, zc);
      zc += zd;
    }
    xc += xd;
  }
  // Match SS_Background: GetNormalizedData(false) then sample [y, x]
  const out = new Float32Array(width * height);
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      const sample = (uc[y + x * ucWidth] + 1) * 0.5;
